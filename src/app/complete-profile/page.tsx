...
export default function CompleteProfilePage() {
  ...
  const handleComplete = async (e: React.FormEvent) => {
    ...
    // Try to ensure user_roles exists (defaulting to student)
    await supabase
      .from("user_roles")
      .insert({
        user_id: user.id,
    ...
    // Check if the role is admin to redirect accordingly
      ...
      <div className="w-full max-w-md bg-[#0D2038] border border-white/10 rounded-3xl p-6 sm:p-8 shadow-2xl relative z-10 space-y-6">
        ...
        <form onSubmit={handleComplete} className="space-y-4">
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full py-3.5 text-xs font-bold text-slate-950 bg-[#F59E0B] hover:bg-[#FACC15] rounded-xl transition-all flex items-center justify-center gap-2 disabled:opacity-50"
          >
